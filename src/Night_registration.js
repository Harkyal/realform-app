...
function Night_registration(props) {
  const user = getUser();
  ...
  var [Operator_list, setOperator_list] = useState(Night_operators);
  ...
  const submitValue2 = (e) => {
    ...
    setOperator({ Operator: Night_operators.find((x) => x.Operator === e.target.value).Operator });
        ...
        <div class="row">
          ...
          <div class="col">
            ...
            <datalist id="operatorList">
              <option></option>
              {Night_operators.map(x => {
                return <option>{x.Night_Operators}</option>
              })}
            </datalist>
...
export default Night_registration;

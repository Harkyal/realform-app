...
function CascadingDropdown(props) {
  const user = getUser();
  ...
  const submitPost = () => {
    ...
    Axios.post('http://localhost:3002/api/create', { Date: Date2, Operator: JSON.parse(Operator_value), Part_internalNo: JSON.parse(Part_internalNo_value), PartNo: JSON.parse(PartNo_value), Customer: JSON.parse(Customer_value), Part_description: JSON.parse(Part_Description_value), Target: Target, work_station: work_station, Produced: Produced, Scrap: Scrap, Supervisor_notes: Supervisor_notes, Operator_2: Operator_2 })
